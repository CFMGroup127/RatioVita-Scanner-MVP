...
enum SampleSeed {
    ...
    /// Arthur Jensen compliance injection: single test receipt for ledger verification.
    /// Merchant: Sovereign Coffee Co., Total: $42.39, Date: Feb 02, 2026 (or today 14:45).
    static func insertSovereignCoffeeTestReceipt(into context: ModelContext) {
        let calendar = Calendar.current
        let createdAt = calendar.date(bySettingHour: 14, minute: 45, second: 0, of: Date()) ?? Date()
        let receipt = Receipt(
            createdAt: createdAt,
        ...
        let rv = placeholderReceiptImage(width: 900, height: 1400, title: "Sovereign Coffee Co.", page: 1)
        let ocrText = makeOCRText(merchant: "Sovereign Coffee Co.", total: receipt.total, date: createdAt, page: 1)
        let img = ReceiptImage(
            pageIndex: 0,
            image: rv,
            ocrText: ocrText,
