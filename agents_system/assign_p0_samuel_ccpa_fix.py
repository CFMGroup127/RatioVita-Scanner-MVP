...
def assign_p0_ccpa_fix_to_samuel():
    ...
    # Load tools for Samuel (including execution tools for P4)
    from tools import (
...
**STEP 3: P4 AUTONOMOUS EXECUTION (MANDATORY)**
After completing P3 logging, you MUST immediately execute the task:

**EXECUTION STEPS:**
1. **LOCATE THE FILE:**
   - Use FileReadTool to search for `data_processor.py` in the codebase
   - Identify the data processing library being used
   - Check the current library version

2. **UPDATE THE LIBRARY:**
   - Research the latest version with authenticated logging support
   - Update the library dependency
   - Verify the new version includes the logging hook

3. **INTEGRATE THE LOGGING HOOK:**
   - Use FileReadTool to read the current implementation
   - Use FileWriteTool or CursorLLMTool to implement the hook
   - Ensure CCPA compliance requirements are met

4. **VERIFY COMPLIANCE:**
   - Test the logging hook
   - Verify it captures all required audit trail data
   - Confirm CCPA compliance is restored

5. **LOG COMPLETION:**
   - Update memory document with completion status
   - Mark Google Task as COMPLETE
   - Include artifact references (file paths, changes made)

**CRITICAL:** This is a P0 blocker task. You must EXECUTE the fix immediately after P3 logging, not just acknowledge it. Use your available tools (CodeInterpreterTool, FileReadTool, FileWriteTool, CursorLLMTool) to actually implement the solution.

**OUTPUT:**
After completing P3 + P4 protocols, provide confirmation:
1. Task logged to memory document (TASKS section) - SUCCESS
2. Task created in Google Tasks - SUCCESS
3. Task EXECUTED - File modified, library updated, logging hook implemented
4. Task COMPLETE - Verified, logged, and marked complete with artifact references
"""
    
    task = Task(
        description=task_description,
        agent=samuel_agent,
        expected_output=f"P3 + P4 protocols completed: P0 task '{task_details['task_name']}' logged to memory document (TASKS section) and created in Google Tasks. Task EXECUTED: authenticated logging hook implemented, library updated, CCPA compliance verified. Task marked COMPLETE with artifact references."
    )
