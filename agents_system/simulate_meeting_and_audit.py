...
def simulate_meeting_and_audit():
    ...
    # Get key agents and load their tools
    dana_agent = None
    arthur_agent = None
    ...
    for agent in agents:
        ...
        agent_tools = []
        try:
            agent_tools.append(get_google_docs_memory_tool())
        except:
            pass
        try:
            agent_tools.append(get_google_tasks_tool())
        except:
            pass
        try:
            agent_tools.append(get_memory_search_tool())
        except:
            pass
...
[10:16 AM] Dana Flores: Will do. [P13 Acknowledgment] The Executive Strategy Report will be generated and emailed to the Human Owner by EOD, synthesizing data from all agent logs."""
    ...
    try:
        # Use CrewAI agents to execute the logging tasks
        # This ensures tools are called correctly within agent context
        from crewai import Task
        
        dana_task1 = Task(
            description=f"""
Use the Google Docs Memory Tool to log FULL MEETING MINUTES to your memory document:
- doc_id: {dana_doc_id}
- section: "MEETINGS"
- subsection: "November 25, 2025"
- template: "MEETING_MINUTES"
- content: {full_minutes[:500]}...
""",
            agent=dana_agent,
            expected_output="Full meeting minutes logged to MEETINGS section"
        )
        
        dana_task2 = Task(
            description=f"""
Use the Google Docs Memory Tool to log FULL MEETING TRANSCRIPT to your memory document:
- doc_id: {dana_doc_id}
- section: "TRANSCRIPTS"
- subsection: "November 25, 2025"
- template: "MEETING_TRANSCRIPT_ARCHIVE"
- content: {full_transcript[:500]}...
""",
            agent=dana_agent,
            expected_output="Full meeting transcript logged to TRANSCRIPTS section"
        )
        
        crew = Crew(agents=[dana_agent], tasks=[dana_task1, dana_task2], verbose=False)
        result = crew.kickoff()
        
        print(f"✅ Dana logged FULL MEETING MINUTES and TRANSCRIPT")
...
- CCPA compliance is a critical blocker for V2 feature implementation"""
    ...
    try:
        ...
        arthur_task = Task(
            description=f"""
Use the Google Docs Memory Tool to log BRIEF ROLE-SPECIFIC meeting notes (NOT full minutes):
- doc_id: {arthur_doc_id}
- section: "MEETINGS"
- subsection: "November 25, 2025"
- template: "Meeting Notes"
- content: {role_specific_notes}
- CRITICAL: This must be BRIEF (under 150 words) and role-specific only
""",
            agent=arthur_agent,
            expected_output="Brief role-specific meeting notes logged to MEETINGS section"
        )
        
        crew = Crew(agents=[arthur_agent], tasks=[arthur_task], verbose=False)
        result = crew.kickoff()
        
        print(f"✅ Arthur logged BRIEF ROLE-SPECIFIC NOTES")
...
- Protocol: P3 Sign-Off Required"""
    ...
    try:
        ...
        arthur_task1 = Task(
            description=f"""
Use the Google Docs Memory Tool to log the assigned task to your TASKS section:
- doc_id: {arthur_doc_id}
- section: "TASKS"
- subsection: "November 25, 2025"
...
""",
            agent=arthur_agent,
            expected_output="Task logged to TASKS section in memory document"
        )
        
        # PART B: Google Tasks (Human-Interactive)
        arthur_task2 = Task(
            description="""
Use the Google Tasks Tool to create the task in Google Tasks (Hybrid System Part B):
...
""",
            agent=arthur_agent,
            expected_output="Task created in Google Tasks Sidebar"
        )
        
        crew = Crew(agents=[arthur_agent], tasks=[arthur_task1, arthur_task2], verbose=False)
        result = crew.kickoff()
        
        print(f"✅ PART A: Task logged to MEMORY DOCUMENT")
        print(f"✅ PART B: Task created in GOOGLE TASKS")
        print(f"   Result: {str(result)[:200]}...")
